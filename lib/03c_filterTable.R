filterTable <- function(genomic.coordinate, DNA.pattern, strand.sensitive,
                        output, env=parent.frame()) {
  ...
  cat("\n\nSummary of data that are removed from the genomic",
      "coordinate table.\n", sep = "")
  cat("-------------------------------------------------------------------\n")
  ...
  if (!is.null(DNA.pattern)) {
    ...
    if (nrow(pattern.count[(!sequence %in% DNA.pattern)]) > 0) {
      ...
      pattern.percent <- env[[genomic.coordinate]][(!sequence %in% DNA.pattern),
                     table(sequence) / nrow(env[[genomic.coordinate]]) * 100]

      cat("\nThe following percentage of unwanted sequence pattern are",
          "removed:\n", sep = "")
  ...
  if (env[[genomic.coordinate]][chromosome %in% mito, .N] > 0) {
    mito.percent <- env[[genomic.coordinate]][chromosome %in% mito, .N] /
        nrow(env[[genomic.coordinate]]) * 100
  } else {
    mito.percent <- 0
  }
  ...
  if (strand.sensitive & (env[[genomic.coordinate]][strand == "*", .N] > 0 |
       sum(duplicated(env[[genomic.coordinate]][, .(chromosome, start, end)])) >
         0) ) {
    message(paste0("The strand mode is sensitive but there is information",
                   "on both strand."))

    dup.idx <- duplicated(env[[genomic.coordinate]][,
                          .(chromosome, start, end)]) |
               duplicated(env[[genomic.coordinate]][,
                          .(chromosome, start, end)], fromLast = TRUE)
  ...
  # Save removed data
  if (!is.null(DNA.pattern)) {
    total.remove <- env[[genomic.coordinate]][dup.idx | chromosome %in% mito |
                                              (!sequence %in% DNA.pattern), .N]
    fwrite(env[[genomic.coordinate]][dup.idx | chromosome %in% mito |
           (!sequence %in% DNA.pattern)], paste0(output, "/removed_table.csv"))
  } else if (is.null(DNA.pattern)) {
    total.remove <- env[[genomic.coordinate]][dup.idx | chromosome %in%
                                              mito, .N]
    fwrite(env[[genomic.coordinate]][dup.idx | chromosome %in% mito],
  ...
  if (!is.null(DNA.pattern)) {
    env[[genomic.coordinate]] <- env[[genomic.coordinate]][(!dup.idx) &
                                 (!chromosome %in% mito) &
                                 (sequence %in% DNA.pattern)]
  } else if (is.null(DNA.pattern)) {
    env[[genomic.coordinate]] <- env[[genomic.coordinate]][(!dup.idx) &
                                 (!chromosome %in% mito)]
  }
