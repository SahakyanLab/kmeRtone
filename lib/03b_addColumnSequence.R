addColumnSequence <- function(genomic.coordinate, genome, shrink=FALSE,
                              env=parent.frame()) {
  # Because data.table is already multithreading, we can only see faster speed
  # after 1M table row when using foreach loop
  #
  # shrink      <bool>     To shrink coordinate region to point to DNA pattern
  #                        at the center. e.g. UV data
  ...
  if (nrow(env[[genomic.coordinate]][strand == "*"]) > 0) {
    message(paste0("WARNING! There is same genomic coordinate on plus",
                   "and minus strands."))
    env[[genomic.coordinate]] <-
        rbind(env[[genomic.coordinate]][strand %in% c("+", "-")],
              env[[genomic.coordinate]][strand == "*"][, strand := "+"],
              env[[genomic.coordinate]][strand == "*"][, strand := "-"])
    env[[genomic.coordinate]] <-
        unique(env[[genomic.coordinate]][, .(chromosome, start, end, strand)])

    gc()
  }

  # for shrinking original coordinate that spans flanking region of
  # case position e.g. uv data.
  ...
  #
  #   genomic.coordinate.list <-
  #       lapply(1:ncpu, function(i) {
  #          env[[genomic.coordinate]][ chunks$start[i]:chunks$end[i] ])}
  #
  #   # map the sequence
  #   toExport <- c("reverseComplement")
  #   seqs <- foreach(genomic.coordinate=genomic.coordinate.list,
  #                   .combine = "c", .noexport = ls()[!ls() %in% toExport],
  #                   .packages = "data.table") %dopar% {
  #
  #   # divide table by 10mil
  #   #table.chunks <- gl(env[[genomic.coordinate]][, .N/5e+6], 5e+6,
  #                       env[[genomic.coordinate]][, .N])
  #
  #   # get sequence
  #   env[[genomic.coordinate]][, sequence := {
  #     if (.N > 7e+5) {
  ...
  #     dna.seq <- unlist(stri_sub_all(genome[[chromosome]], start, end))
  #     if (strand == "-") {
  #         dna.seq <- reverseComplement(dna.seq, form = "string")}
  #
  #     dna.seq
  #      },by = .(chromosome, strand)]
  #
