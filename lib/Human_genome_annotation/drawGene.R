drawGene <- function(genomic.coordinate, colours=NULL){
  ...
  elements <- c("upstream", "UTR5", "CDS", "intron", "UTR3", "downstream", "UTR")
  ...
  ybottoms <- lapply(elements, function(element_) ybottom[dt$element == element_])
  ...
  for (name_ in dt[, unique(name)]){
    ...
    text(x=dt[name == name_, min(start)] - 0.5, y=min(ybottoms_)+i-0.05, labels = name_, adj = 0, cex = 0.8)
