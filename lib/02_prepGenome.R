prepGenome <- function(genome.name, genome.path, genome.prefix,
                       genome.suffix, genome, env=parent.frame()) {
  ...
  if (class(genome) == "genome") {
    ...
    if (grepl("[RDSrds]", genome.path)) {
      ...
      cat("\n -- Genome is saved as RDS file at:",
          paste0(genome.path, "/", genome.name, ".rds."))
    ...
    genome.path <- paste0("data/genome/human/", genome.name, "/", genome.name,
                          ".rds")
    ...
    if (!file.exists(genome.path)) {
      ...
      fasta.not.exist <- length(list.files(genome.folder,
                                           pattern = "chr.+[Ff]a")) < 23

      # Download
      # The ftp storage hierachy is not consistent for hg19 and hg38. Some
      # genome is an old version. They are all over the place. I had to manually
      # hunt down the url for the latest version.
      if (fasta.not.exist) {
        ...
        if(genome.name == "hg38"){
          ...
          # This url link to a compressed folder "chroms" containing
          # uncompressed fasta files.
          url.path <- paste0("http://hgdownload.soe.ucsc.edu/goldenPath/",
                             genome.name, "/bigZips/latest/hg38.chromFa.tar.gz")
          download.file(url.path, paste0(genome.folder, "hg38.chromFa.tar.gz"),
                        quiet = TRUE, method = 'curl')

          # Extract
          untar(paste0(genome.folder, "hg38.chromFa.tar.gz"),
                exdir = genome.folder)

          # Move file from genome/chroms folder to genome/
          chr.filenames <- list.files(paste0(genome.folder, "chroms/"))
          file.rename(paste0(genome.folder, "chroms/", chr.filenames),
                      paste0(genome.folder, chr.filenames))
          ...
          url.path <- paste0("http://hgdownload.cse.ucsc.edu/goldenPath/",
                             genome.name, "/bigZips/hg19.fa.gz")
          download.file(url.path, genome.fasta.path, quiet = TRUE)

          splitFasta(genome.fasta.path, genome.folder, compression = 1)

          chr.filenames <- list.files(genome.folder,
                                      "chr[0-9A-Za-z_]+\\.(fa|fna|fasta)")
        }
      }

      cat("DONE!\n -- Building genome...")
      # Build genome
      env$genome <- loadGenome(genome.folder, genome.name, env,
                               save.as.RDS = TRUE)
      cat("DONE!\n -- Loading genome...")
      ...
      if ("chr.filenames" %in% ls()) {
        ...
        suppressWarnings(file.remove(paste0(genome.folder, genome.name,
                                            ".fa.gz")))
      }
      # Write a text file containing record of current chromosomes in
      # genome.RDS (chromosome name + length)
      chr.info <- data.table(chromosome = names(env$genome),
                             length = attr(env$genome, "length"))
