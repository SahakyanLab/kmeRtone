getCaseKmers <- function(genomic.coordinate, genome, k, DNA.pattern,
                         strand.sensitive, remove.overlaps = TRUE,
                         env = parent.frame()) {

  # Dependencies:
  #     Kmertone variables: genomic.coordinate, DNA.pattern, k, strand.sensitive
  #                         kmertone.env, genome
  ...
  if (length(len) == 1 & len < k) {
    ...
    cat("--Coordinate lengths are the same or bigger than size k.\n--Kmer",
        "extraction will be performed by sliding window of size k.\n")
  }
  ...
  if (remove.overlaps) {
    ...
    before <- env[[genomic.coordinate]][end - start + 1 >= k, .N]

    removeAllOverlaps("genomic.coordinate", env, remove = FALSE,
                      strand.sensitive = FALSE)

    loss <- (before - env[[genomic.coordinate]][!is.na(end - start) &
             end - start + 1 >= k, .N]) / before * 100
  ...
  if (!strand.sensitive) countReverseComplementKmers("case.kmers")
